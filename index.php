<?php
	...
	// list of items
	if ( !$db->query("create table if not exists items(id integer primary key, item text, complete integer, whichlist integer, foreign key(whichlist) references lists(id))") ) {
	...
	//***** check for our lists
	if ($theselists) {
		//***** but only if we're not looking at a list or editing items
		// super mega clusterfuck goooo
		if (
			$mode != "viewlist" && 
			$mode != "additem" && 
			$mode != "edititem" &&
		...
		else { // begin mode for listing the items of the selected list
	...
	//***** oy vey what have i gotten myself into with these modes
	if (
		$mode && 
		$mode != "addlist" &&
		$mode != "editlist" &&
		...
		echo "What are you doing?</br></br>";
	}
	
	
	//********************** list modes
	
	//***** let's try to add a list
	if ($mode == "addlist") {
	...
	//***** delete list
	// we have to delete the items, then the list itself
	...
	//***** edit task, but also make sure we've selected an item, and that the item exists
	if ($mode == "edititem") {
	...
	//***** switch check/not checked status
	// make sure we have a good list item, then if we're in check mode just swap the value
	if ($mode == "checkitem") {
